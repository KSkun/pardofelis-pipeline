...
fn getLightResult(
  ...
  var f0 = getF0(matParam.color, matParam.metallic);
  ...
  var diffuse = kDiffuse * matParam.color / pi;
...
fn main(
  ...
  for (var i : u32 = 0; i < pointLights.size; i++) {
    ...
    lightResult += getLightResult(worldPos, normal, cameraPos, material, lightParam);
  }
  lightResult += ambient * material.color * material.ambientOcc;
